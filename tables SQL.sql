...
CREATE OR REPLACE TABLE t_huu_viet_nguyen_project_SQL_primary_final AS (
    SELECT 
        cpay.payroll_year,
        cpay.ib_name,
        cpay.ib_code,
    ...
    FROM (
        ...
        GROUP BY food_code, YEAR(cp.date_from), food_name, price_value, price_unit
    ) cp
    INNER JOIN (
        SELECT 
    ...
    ON cp.price_year = cpay.payroll_year
);

CREATE OR REPLACE TABLE t_huu_viet_nguyen_project_SQL_secondary_final AS (
    SELECT 
        c.country,
        c.capital_city, 
        e.year, 
